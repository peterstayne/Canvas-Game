...
b.x<w.x+(b.size>>1)&&(b.x>w.x-(b.size>>1)&&b.y<w.y+(b.size>>1)&&b.y>w.y-(b.size>>1))&&(clearInterval(H),I=d))},h:function(){var a=1,b,e,k,n,r;p&&(q.shadowBlur=5,q.shadowOffsetX=3,q.shadowOffsetY=3);for(var f in this.b)if(void 0!==this.b[f])if(r=this.b[f].size,"dead"!==this.b[f].status)p&&(q.shadowColor="rgba(0,0,0,1)"),"wounded"!==this.b[f].status?n=this.b[f].color+",1)":(this.b[f].status="alive",n="rgba(255,255,255,1)"),q.fillStyle!==n&&(q.fillStyle=n),q.fillRect(~~this.b[f].x-(r>>1),~~this.b[f].y-
(r>>1),r,r);else{var P=this.b[f].color;100===this.b[f].c&&(P="rgba(255,255,255");this.b[f].c-=0.1*y;n=this.b[f].c;if(0>=n)delete this.b[f];else{p&&(q.shadowColor="rgba(0,0,0,0)");if(90<n){a=3;q.strokeStyle="rgba(255, 255, 0, 1)";q.lineWidth=4;q.beginPath();e=Math.random()*(110-n);for(k=Math.random()*(140-n);--a;)b=~~(628*Math.random()-314),q.moveTo(~~(this.b[f].k.x+A[b]*e),~~(this.b[f].k.y+B[b]*e)),q.lineTo(~~(this.b[f].k.x+A[b]*k),~~(this.b[f].k.y+B[b]*k));q.stroke();q.closePath()}cacheIndex=~~(100*
this.b[f].a);this.b[f].x+=A[cacheIndex]*0.03*n;this.b[f].y+=B[cacheIndex]*0.03*n;a=(n/100).toFixed(2);q.fillStyle=P+","+a+")";r+=~~(r*((100-n)/100));q.fillRect(~~this.b[f].x-(r>>1),~~this.b[f].y-(r>>1),r,r)}}}}}
function J(){if(!I||K)return d;var a=Date.now();y=a-gameClock;v+=2E-5*y;L++;M+=y;gameClock=a;w.t();C.t();z.h();C.h();w.h();x.h();j({color:"#f0f",text:"Score: "+u,x:80,y:30});a=~~(L/(M/1E3));j({color:"#f0f",text:"FPS: "+a,x:80,y:55});p&&(50>a&&1E4<M)&&(q.shadowColor="rgba(0,0,0,0)",p=d);I||s();requestAnimationFrame(J)}var N,q,w={},z={},C={},v=0.7,u=0,H,I=d,K=d,D=Math.PI,F=Math.PI/2,E=2*Math.PI,l,m,B,A,O=200*E>>0,x={},y=0,L=0,M=0,p=c,G=0;gameClock=Date.now();A=[];
for(var Q=-O;Q<O;Q++)A[Q]=Math.sin(Q/100);B=[];for(Q=-O;Q<O;Q++)B[Q]=Math.cos(Q/100);l=900;m=660;N=document.getElementById("gamecanvas");q=N.getContext("2d");bgcanvas=document.getElementById("bgcanvas");bgctx=bgcanvas.getContext("2d");N.setAttribute("width",l);N.setAttribute("height",m);bgcanvas.setAttribute("width",l);bgcanvas.setAttribute("height",m);
function R(){var a=document.getElementById("bgimg");z.r={B:a,width:a.width,height:a.height};bgctx.drawImage(z.r.B,0,0,z.r.width,z.r.height,0,0,l,m)}document.getElementById("bgimg").onload=R;document.getElementById("bgimg").complete&&R();z={width:l,height:m,offset:{left:window.innerWidth/2-N.width/2,top:N.offsetTop},C:"gravel.png",h:function(){p&&(q.shadowColor="rgba(0,0,0,0)");N.width=N.width}};
