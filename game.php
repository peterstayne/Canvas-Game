<?php
	...
	$height = 510;
                     ...
                     //                 cpi - 3.14 ... pointing up      
                     // cpi3  4.71 ... pointing left     cpi2  1.57 ... pointing right
                     //               cpi360/0  6.28 . pointing down
...
var canvas, ctx, player = {}, field = {}, bullets = [], enemies = [], frame = 0.7, score = 0, enemy_types = [], to, gameOn = false;
var cpi = Math.PI, cpi2 = Math.PI / 2, cpi4 = Math.PI / 4, cpi3 = cpi + cpi2, cpi360 = Math.PI * 2;
var bgimg = '';
...
function resetGame() {
   ...
   frame = 0.7;
   player = {
      x: <?php echo floor($width / 2); ?>,
      y: <?php echo floor($height / 2); ?>,
      hp: 100,
      moveUp: false,
      moveDown: false,
      moveLeft: false,
		...
		for(var i in bullets) {
			...
			for(var j in enemies) {
				...
				if(bullets[i] !== undefined &&
				   bullets[i].x < enemies[j].x + size && 
				   bullets[i].x > enemies[j].x - size && 
				   bullets[i].y < enemies[j].y + size && 
				   bullets[i].y > enemies[j].y - size &&
				   enemies[j].hp > 0) {
				   if(!--enemies[j].hp) {
		...
		for(var i in enemies) {
		   if(enemies[i] !== undefined) {
            ...
            if(enemies[i].hp > 0) {
               ...
               if(
                  enemies[i].x < player.x + (size >> 1) && 
                  enemies[i].x > player.x - (size >> 1) && 
                  enemies[i].y < player.y + (size >> 1) && 
                  enemies[i].y > player.y - (size >> 1) &&
                  enemies[i].hp > 0) {
                  clearInterval(to);
                  fail();
